...
const ShiftView = () => {
  ...
  //Carga de datos
  useEffect(() => {
    getPagin('http://192.168.20.244:5000/api/ShiftPatterns',page, setTotalPages, setData, setLoading, recordsPerPage);
    }, [page]
  );
  ...
  useEffect(() => {
    if (search.trim() === '') {
      ...
      paginSearch('http://192.168.20.244:5000/api/ShiftPatterns', 'name', search, page, setTotalPages, setFilteredData, setLoading, recordsPerPage);
    }
  }, [search, data, page]);
    ...
    <View>
      ...
      <View style={styles.card}>
            ...
            <FlatList
              ...
              keyExtractor={(item) => item.idShiftPattern.toString()}
